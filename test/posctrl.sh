...
# 兜底时区自检 ①：拆掉「时区库里真有这份定义」那一步，等于退回 2026-08-31 之前那版
# 「只要能取到 %z 就放行」——而 `+0000` 正是解析不到时的降级结果 ⇒ 事故 (a) 原样复活。
# ⚠️ 这条消融动的是**触发条件**（哪些规格算可用），不是把守卫整个拆掉：函数仍在、仍被调用、
#    仍会对取不到偏移量的情况返回失败——只是恢复了那条恒真的判据。
ablate tz-spec-unresolvable \
  "解析不到的兜底 TZ 必须判失败，不许静默当 UTC" --fast \
  "解析不到的 TZ 规格被当成可用时区" \
  m_sed lib/config.sh \
    '  [[ -f "$file" && -r "$file" ]] && return 0' \
    '  return 0'

# 兜底时区自检 ②：退回**形态判据**——「有斜杠就行 / 认一张 UTC 别名表」。
# 🩸 这不是假想的退化，这是本函数 2026-08-31 第一版真实发的版本，由 review 实测揪出：
#    它放过 `Etc`（是目录）、`Z`（库里没有）、`Asia/Nowhere`、`Foo/Bar`（拼写规整但没东西
#    回应），四个全部静默 `+0000`；同时误拒 **31 个真实时区**——`Japan`/`EST`/`Iran`/`CET`/
#    `Poland` 这些名字就是不带斜杠的普通词。
# ⭐ 留这条消融的理由：**名字长得规整，不说明有东西回应它**。下一个改这里的人最可能
#    重新掉进的就是这个坑，而这条消融会同时从**两侧**（误收 + 误拒）把他拦下来。
ablate tz-spec-form-only \
  "判据必须是可解析性，不是规格的形态（误收与误拒两侧都要拦）" --fast \
  "解析不到的 TZ 规格被当成可用时区" \
  m_sed lib/config.sh \
    '  [[ -f "$file" && -r "$file" ]] && return 0' \
    '  case "$spec" in UTC|GMT|UCT|Z|Zulu|Universal|Etc) return 0 ;; esac; [[ "$spec" == */* ]] && return 0'
